...
func CreateTodo(pool *pgxpool.Pool, title string, completed bool, userID string) (*models.Todo, error) {
	...
	//we use context to communicate with the database
	var ctx context.Context
	var cancel context.CancelFunc
	ctx, cancel = context.WithTimeout(context.Background(), 5*time.Second) //we dont want the database connection to run forever
	defer cancel()
	...
	//acquires conenction and executes the query
	//scan is used to store this query data into a Go Struct
